<x-app-layout>
                ...
                <div class="p-6">
                    ...
                    <div class="bg-gray-50 overflow-hidden shadow-sm sm:rounded-lg p-6 mb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            <div>
                                <span class="block text-sm font-medium text-gray-500">ReferÃªncia</span>
                                <span class="block mt-1 text-sm text-gray-900">{{ $produto->referencia }}</span>
                            </div>

                            <div>
                                <span class="block text-sm font-medium text-gray-500">DescriÃ§Ã£o</span>
                            ...
                            <div>
                                <span class="block text-sm font-medium text-gray-500">LocalizaÃ§Ã£o Atual</span>
                                <span class="block mt-1">
                                    ...
                                    @else
                                        <span class="text-gray-400 text-sm italic">NÃ£o localizado</span>
                                    @endif
                                </span>
                            </div>

                            <div>
                                <span class="block text-sm font-medium text-gray-500">PreÃ§o Atacado</span>
                                <span class="block mt-1 text-sm text-gray-900">R$ {{ number_format($produto->preco_atacado, 2, ',', '.') }}</span>
                            </div>

                            <div>
                                <span class="block text-sm font-medium text-gray-500">PreÃ§o Varejo</span>
                            ...
                            <div>
                                <span class="block text-sm font-medium text-gray-500">Ãšltima atualizaÃ§Ã£o</span>
                                <span class="block mt-1 text-sm text-gray-900">{{ $produto->updated_at->format('d/m/Y H:i') }}</span>
                            </div>

                            @if($produto->deleted_at)
                                <div>
                                    <span class="block text-sm font-medium text-gray-500">ExcluÃ­do em</span>
                                    ...
                                    <div class="overflow-x-auto">
                                        <table class="min-w-full divide-y divide-gray-200">
                                            <thead class="bg-gray-50">
                                                <tr>
                                                    <th scope="col" class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">DescriÃ§Ã£o</th>
                                                    <th scope="col" class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ReferÃªncia</th>
                    ...
                    <!-- VariaÃ§Ãµes de Cores -->
                     <div class="bg-gray-50 overflow-hidden shadow-sm sm:rounded-lg p-6 mb-6">
                         <h3 class="text-lg font-semibold text-gray-800 mb-4">VariaÃ§Ãµes de Cores</h3>
                                 ...
                                 <table class="min-w-full divide-y divide-gray-200">
                                     <thead class="bg-gray-50">
                                         <tr>
                                             ...
                                             <th scope="col" class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">CÃ³digo</th>
                                         ...
                                         <tr>
                                             ...
                                             <td class="px-4 py-2 whitespace-nowrap text-sm font-bold text-gray-900">
                                                 <div class="flex items-center">
                                                     ...
                                                     @endphp
                                                     @if($isEqual)
                                                         <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-green-500" viewBox="0 0 20 20" fill="currentColor" title="Total das cores estÃ¡ igual Ã  quantidade do produto ({{ number_format($quantidadeProduto, 0, ',', '.') }})">
                                                             <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                                                         </svg>
                                                     @else
                                                         <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-red-500" viewBox="0 0 20 20" fill="currentColor" title="Total das cores ({{ number_format($totalCores, 0, ',', '.') }}) Ã© diferente da quantidade do produto ({{ number_format($quantidadeProduto, 0, ',', '.') }})">
                                             ...
                                             <tr>
                                                 <td colspan="3" class="px-4 py-2 text-xs text-gray-600 bg-yellow-50">
                                                     <div class="flex items-center">
                                                         ...
                                                         <span>Quantidade do produto: {{ number_format($quantidadeProduto, 0, ',', '.') }} | DiferenÃ§a: {{ number_format(abs($totalCores - $quantidadeProduto), 0, ',', '.') }}</span>
                                                     </div>
                                                 </td>
                                             </tr>
                                         @endif
                         ...
                         @else
                             <span class="text-gray-400 italic">Nenhuma variaÃ§Ã£o de cor definida para este produto</span>
                         @endif
                     </div>
                    ...
                    <div id="modal-adicionar-anexo" class="fixed inset-0 bg-gray-500 bg-opacity-75 z-50 hidden overflow-y-auto">
                        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
                            ...
                            <form action="{{ route('produtos.anexos.store', $produto->id) }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="px-6 py-4">
                                    <div class="mb-4">
                                        <label for="descricao" class="block text-sm font-medium text-gray-700 mb-1">DescriÃ§Ã£o</label>
                                    ...
                                    <div>
                                        ...
                                        <p class="mt-1 text-sm text-gray-500">Formatos aceitos: PNG, JPG, JPEG (mÃ¡x. 10MB) e PDF (mÃ¡x. 1MB)</p>
                                    </div>
                                </div>
                    ...
                    <!-- Carrossel de Imagens das MovimentaÃ§Ãµes -->
                    @php
                    ...
                    <div class="bg-gray-50 overflow-hidden shadow-sm sm:rounded-lg p-6 mb-6">
                        <h3 class="text-lg font-semibold text-gray-800 mb-4">Imagens das MovimentaÃ§Ãµes</h3>
                                    ...
                                    @foreach($movimentacoesComAnexo as $index => $movimentacao)
                                        <div class="carousel-item min-w-full flex flex-col items-center justify-center" data-index="{{ $index }}">
                                            <div class="relative w-full max-w-2xl">
                                                <img src="{{ $movimentacao->anexo_url }}" alt="Anexo da movimentaÃ§Ã£o" class="w-full h-auto max-h-96 object-contain rounded-lg shadow-md">
                    ...
                    <div class="bg-gray-50 overflow-hidden shadow-sm sm:rounded-lg p-6">
                        @php
                        // FunÃ§Ã£o para calcular dias Ãºteis entre duas datas (excluindo sÃ¡bados e domingos)
                        ...
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-lg font-semibold text-gray-800">MovimentaÃ§Ãµes</h3>
                                            ...
                                            <tr class="hover:bg-gray-50">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                                    @php
                                                        ...
                                                        // Calcular dias Ãºteis entre DATA ENTRADA e DATA SAIDA (ou data atual se nÃ£o houver saÃ­da)
                                                        if ($movimentacao->data_entrada) {
                                                            if ($movimentacao->data_saida) {
                                                                // Se tem data de saÃ­da, calcular dias Ãºteis entre entrada e saÃ­da
                                                                $diasEntre = calcularDiasUteis($movimentacao->data_entrada, $movimentacao->data_saida);
                                                            } else {
                                                                // Se nÃ£o tem data de saÃ­da, calcular dias Ãºteis entre entrada e data atual
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    <span class="px-2 py-1 rounded-full text-xs {{ $movimentacao->comprometido ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800' }}">
                                                        {{ $movimentacao->comprometido ? 'Sim' : 'NÃ£o' }}
                                                    </span>
                                                </td>
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                                    <a href="{{ route('movimentacoes.show', $movimentacao->id) }}" class="text-blue-600 hover:text-blue-900" title="Visualizar movimentaÃ§Ã£o">
                        ...
                        @else
                            <div class="text-gray-500 italic">
                                Nenhuma movimentaÃ§Ã£o encontrada para este produto.
                            </div>
                        @endif
                    </div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Eventos para os botÃµes de navegaÃ§Ã£o
            if (prevButton) {
