<nav x-data="{ open: false, dropdownOpen: false }" class="bg-white border-b border-gray-100 sticky top-0 z-50 shadow-md">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <a href="{{ route('movimentacoes.minhas') }}" class="relative inline-flex items-center p-2 text-gray-500 hover:text-gray-700 focus:outline-none transition ease-in-out duration-150" title="Minhas Movimentações">
                    ...
                    @endif
                    @if($notificacoesAtrasadas > 0)
                    <span class="absolute bottom-0 right-0 inline-flex items-center justify-center w-3 h-3 transform translate-x-1/2 translate-y-1/2 bg-orange-500 rounded-full border-2 border-white" title="{{ $notificacoesAtrasadas }} atrasadas"></span>
                    @endif
                </a>
            </div>
            @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(auth()->user()->localizacao_id)
            <x-responsive-nav-link :href="route('movimentacoes.minhas')" :active="request()->routeIs('movimentacoes.minhas')" class="flex items-center justify-between">
                <span>{{ __('Minhas Movimentações') }}</span>
                @if($temNotificacoes)
                <div class="flex items-center gap-2">
                    ...
                    <span class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-white bg-orange-500 rounded-full">
                        {{ $notificacoesAtrasadas }} atrasadas
                    </span>
                    @endif
                </div>
                @endif
