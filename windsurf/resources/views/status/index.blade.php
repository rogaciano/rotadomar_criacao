    ...
    <div class="py-8 bg-slate-50 dark:bg-slate-950">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex flex-wrap gap-3 mb-6">
                <a href="{{ route('status.create') }}" class="inline-flex items-center px-6 py-2.5 border border-transparent rounded-xl font-bold text-xs text-white uppercase tracking-widest shadow-lg active:scale-95 transition-all duration-200" style="background: linear-gradient(135deg, #0284c7 0%, #0369a1 100%);">
                    ...
                    <div class="mb-6 bg-slate-100/50 dark:bg-slate-800/50 p-6 rounded-xl border border-slate-200 dark:border-slate-800">
                        <form action="{{ route('status.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <div class="md:col-span-4 flex justify-end space-x-2">
                                ...
                                <a href="{{ route('status.index') }}" class="inline-flex items-center px-4 py-2.5 bg-gray-200 border border-transparent rounded-xl font-bold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-300 focus:bg-gray-300 active:bg-gray-400 transition ease-in-out duration-150">
                                    Limpar
                                </a>
                            </div>
                        </form>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                            ...
                            <tbody class="bg-white dark:bg-slate-900 divide-y divide-gray-200 dark:divide-slate-700">
                                @forelse ($statuses as $statusItem)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-slate-800/50 {{ $statusItem->trashed() ? 'bg-red-50 dark:bg-red-900/20' : '' }}">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <span class="px-2 py-1 rounded-full text-xs {{ $statusItem->ativo ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-200' : 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-200' }}">
                                                {{ $statusItem->ativo ? 'Ativo' : 'Inativo' }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm">
                                            @if($statusItem->calc_necessidade)
                                                <span class="px-2 py-1 rounded-full text-xs bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-200 font-semibold">
                                                    ✓ Sim
                                                </span>
                                            @else
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex item-center justify-center">
                                                <a href="{{ route('status.show', $statusItem->id) }}" class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300 bg-transparent p-1 rounded-full hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-all">
                                                ...
                                                @if (!$statusItem->trashed())
                                                    <a href="{{ route('status.edit', $statusItem->id) }}" class="text-amber-600 hover:text-amber-900 dark:text-amber-400 dark:hover:text-amber-300 bg-transparent p-1 rounded-full hover:bg-amber-100 dark:hover:bg-amber-900/30 transition-all">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    ...
                                                    <form action="{{ route('status.destroy', $statusItem) }}" method="POST" class="inline-block">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 bg-transparent p-1 rounded-full hover:bg-red-100 dark:hover:bg-red-900/30 transition-all" onclick="return confirm('Tem certeza que deseja excluir este status?')">
                                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                ...
                                                @else
                                                    <form action="{{ route('status.destroy', $statusItem) }}" method="POST" class="inline-block">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300 bg-transparent p-1 rounded-full hover:bg-green-100 dark:hover:bg-green-900/30 transition-all" onclick="return confirm('Deseja restaurar este status?')">
                                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="py-6 px-6 text-center text-gray-500 dark:text-gray-400">Nenhum status encontrado.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
