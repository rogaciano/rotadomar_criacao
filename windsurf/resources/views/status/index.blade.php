                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($statuses as $statusItem)
                                    <tr class="{{ $statusItem->trashed() ? 'bg-red-50' : '' }}">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex item-center justify-center">
                                                <a href="{{ route('status.show', $statusItem->id) }}" class="text-blue-600 hover:text-blue-900 bg-transparent p-1 rounded-full hover:bg-blue-100 transition-all">
                                                ...
                                                @if (!$statusItem->trashed())
                                                    <a href="{{ route('status.edit', $statusItem->id) }}" class="text-amber-600 hover:text-amber-900 bg-transparent p-1 rounded-full hover:bg-amber-100 transition-all">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    ...
                                                    <form action="{{ route('status.destroy', $statusItem) }}" method="POST" class="inline-block">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="text-red-600 hover:text-red-900 bg-transparent p-1 rounded-full hover:bg-red-100 transition-all" onclick="return confirm('Tem certeza que deseja excluir este status?')">
                                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                ...
                                                @else
                                                    <form action="{{ route('status.destroy', $statusItem) }}" method="POST" class="inline-block">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="text-green-600 hover:text-green-900 bg-transparent p-1 rounded-full hover:bg-green-100 transition-all" onclick="return confirm('Deseja restaurar este status?')">
                                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="py-6 px-6 text-center text-gray-500">Nenhum status encontrado.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
