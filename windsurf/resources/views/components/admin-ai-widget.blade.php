...
<script>
    window.aiChatRoute = '{{ route("admin.ai-chat.message") }}';
    window.aiFeedbackRoute = '{{ route("admin.ai-chat.feedback", ["historico" => "__ID__"]) }}';
</script>
                    ...
                    <div :class="['max-w-[85%] text-sm p-3 shadow-sm border',
                        ...
                        <template x-if="!msg.isLoading">
                            <div>
                                ...
                                <!-- Feedback: thumbs up/down (apenas para respostas da IA com historicoId) -->
                                <template x-if="msg.role === 'ai' && msg.historicoId && msg.feedback === null">
                                    <div class="mt-2 pt-2 border-t border-slate-100 dark:border-slate-700 flex items-center gap-2">
                                        <span class="text-[10px] text-slate-400">Foi útil?</span>
                                        <button @click="msg.feedback = true; sendFeedback(msg.historicoId, true)"
                                                class="p-1 rounded hover:bg-emerald-50 dark:hover:bg-emerald-900/30 text-slate-400 hover:text-emerald-500 transition-colors"
                                                title="Sim, foi útil">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5"/></svg>
                                        </button>
                                        <button @click="msg.feedback = false; sendFeedback(msg.historicoId, false)"
                                ...
                                <!-- Confirmação após feedback -->
                                <template x-if="msg.role === 'ai' && msg.historicoId && msg.feedback !== null">
                                    <div class="mt-2 pt-2 border-t border-slate-100 dark:border-slate-700">
                                        <span x-text="msg.feedback ? '👍 Obrigado pelo feedback!' : '👎 Anotado, vamos melhorar!'"
