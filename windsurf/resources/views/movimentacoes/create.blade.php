<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('movimentacoes.index') }}" class="btn-ghost-secondary">
                    ...
                    <form action="{{ route('movimentacoes.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="produto_id" name="produto_id" class="select2 block mt-1 w-full border-gray-300 dark:border-slate-600 dark:bg-slate-800 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                            ...
                            <div>
                                ...
                                <input type="datetime-local" name="data_entrada" id="data_entrada" value="{{ old('data_entrada', now()->format('Y-m-d\TH:i')) }}" class="block mt-1 w-full border-gray-300 dark:border-slate-600 dark:bg-slate-800 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                            </div>

                            <!-- Data Conclusão -->
                            <div>
                                <label for="data_saida" class="block text-sm font-medium text-gray-700 mb-1">Data de Conclusão</label>
                                <input type="datetime-local" name="data_saida" id="data_saida" value="{{ old('data_saida') }}" class="block mt-1 w-full border-gray-300 dark:border-slate-600 dark:bg-slate-800 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                            </div>
                            <div>
                                <label for="data_devolucao" class="block text-sm font-medium text-gray-700 mb-1">Data de Devolução </label>
                                <input type="datetime-local" name="data_devolucao" id="data_devolucao" value="{{ old('data_devolucao') }}" class="block mt-1 w-full border-gray-300 dark:border-slate-600 dark:bg-slate-800 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                            </div>

                            <!-- Observação -->
                            <div class="md:col-span-2">
                                <label for="observacao" class="block text-sm font-medium text-gray-700 mb-1">Observação (opcional)</label>
                                <textarea id="observacao" name="observacao" rows="3" class="block mt-1 w-full border-gray-300 dark:border-slate-600 dark:bg-slate-800 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('observacao') }}</textarea>
                            </div>
