    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                ...
                <form action="{{ route('localizacao-capacidade.update', $capacidade->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="md:col-span-2">
                            ...
                            <select id="localizacao_id" name="localizacao_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                ...
                                @foreach($localizacoes as $localizacao)
                                    <option value="{{ $localizacao->id }}" {{ old('localizacao_id', $capacidade->localizacao_id) == $localizacao->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select id="mes" name="mes" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                ...
                                @foreach(range(1, 12) as $m)
                                    <option value="{{ $m }}" {{ old('mes', $capacidade->mes) == $m ? 'selected' : '' }}>
                                        {{ $meses[$m] }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="ano" name="ano" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                ...
                                @foreach(range(now()->year - 1, now()->year + 5) as $a)
                                    <option value="{{ $a }}" {{ old('ano', $capacidade->ano) == $a ? 'selected' : '' }}>
                                        {{ $a }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input id="capacidade" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="number" name="capacidade" value="{{ old('capacidade', $capacidade->capacidade) }}" min="0" required />
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="observacoes" name="observacoes" rows="3" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('observacoes', $capacidade->observacoes) }}</textarea>
                            @error('observacoes')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
